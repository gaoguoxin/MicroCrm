$(->
	...
	$('body').on('click','.search_user',(e)->
		...
		$.get("/manager/users",search_data,->)
	)

	$('body').on('click','td a.delete-user',(e)->
		curr_tr = $(@).parents('tr')
		cid = $(@).data('cid')
		$.get("/manager/users/delete",{id:cid},(ret)->
			if ret.success
				curr_tr.remove()
		)
	)
	...
	$('body').on('click','.pagination a',->
		unless $(@).hasClass('disabled')
			...
			g_data = {page:page,name:name,email:email,position:position,city:city,interest:interest,start:start,end:end}
			if page
				$.get("/manager/users",g_data,->)
	)
	...
	$('body').on('click','a.edit-user',(e)->
		cid = $(@).attr('id')
		$.get("/manager/users/#{cid}/edit",{},(ret)->)
	)
