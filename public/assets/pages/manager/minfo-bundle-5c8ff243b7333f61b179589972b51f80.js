...
(function() {
  $(function() {
    var add_flag, check_password, check_presense, confirm_ipt, delay, email_ipt, flash_notice, go_top, mobile_ipt, name_ipt, password_ipt, remove_flag, send_info_ajax, send_password_ajax;
    name_ipt = $('#name');
    email_ipt = $('#email');
    mobile_ipt = $('#mobile');
    ...
    send_password_ajax = function() {
      return $.ajax({
        ...
        success: function(ret) {
          return flash_notice('密码修改成功!');
        }
      });
    };
    send_info_ajax = function() {
      return $.post('/manager/users/update_info', $('form.info').serialize(), function(ret) {
        if (ret.success) {
          return flash_notice('信息修改成功!');
        } else {
